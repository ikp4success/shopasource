...
This file provides guidance to Gemini Code Assist and Antigravity when working with code in this repository.
...
**Shop selection is LLM-driven, not just keyword extraction.** `parse_nl_query`'s system prompt lists every active shop next to its website domain, and the `shops` field of `_NL_QUERY_SCHEMA` instructs the model to return only the subset that plausibly sells that category of product when the user didn't name a store explicitly (a laptop search shouldn't hit ZARA; a wallet search shouldn't hit NEWEGG) - falling back to every active shop only if the LLM returns nothing. This runs before `ResultsFactory.match_sk()`'s per-item relevance filtering (word-boundary + stopword matching, gated by `MIN_MATCH_ACCURACY` in `webapp/nl_search.py`, never fully disabled) and is a different kind of filter: fewer shops selected means fewer `scrapy crawl` subprocesses spawned for that search at all, which matters for memory given `MAX_CONCURRENT_SHOPS`/`MAX_CONCURRENT_BROWSER_SHOPS` already bound how many can run concurrently. `provider="normal"` always searches every active shop, since there's no LLM to make that judgment call.
