...
class Burlington(ShopBase):
    ...
    def parse_data(self, response):
        ...
        description = self.extract_items(
            response.css(".product-details-description ::text").extract()
        )
        price = response.css(
            "#ctl03_MainContentArea_ctl00_ctl00_ctl00_ProductDisplay_ProductPricesModule_LblOurPriceSimple ::text"
        ).extract_first()
