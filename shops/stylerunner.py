...
class StyleRunner(ShopBase):
    ...
    def parse_results(self, response):
        for item in response.css(".facets-item-cell-grid"):
            item_url = self.prepend_domain(item.css("a ::attr(href)").extract_first(), response.url)
            image_url = item.css(".facets-item-cell-grid-image ::attr(src)").extract_first()
            title = self.extract_items(item.css(".facets-item-cell-grid-title ::text").extract())
            description = title
