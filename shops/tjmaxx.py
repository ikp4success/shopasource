...
class TjMaxx(scrapy.Spider):
    ...
    def get_best_link(self, response):
        item_url = response.css(".product-image a.product-link ::attr(href)").extract_first()
        item_urls = response.css(".product-inner")
        for item in item_urls:
            item_text = item.css(".product-title ::text").extract_first() or ""
            if self._search_keyword.lower() in item_text.lower():
                best_item_link = item.css(".product-image a.product-link ::attr(href)").extract_first() or ""
