...
class DicksSportingGoods(ShopBase):
    ...
    def parse_data(self, response):
        ...
        description = self.extract_items(response.css("div[itemprop='description'] ::text").extract())
        price = (
            response.css("span[itemprop='price'] ::text").extract_first() or
            self.safe_grab(response.meta, "p")
        )
