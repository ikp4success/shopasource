...
class Asos(scrapy.Spider):
    ...
    def parse_data(self, response):
        image_url = response.css(".amp-frame .fullImageContainer img ::attr(src), .product-carousel img ::attr(src)").extract_first() or \
        ...
        price = response.xpath("//span[@itemprop='price']").css("::text").extract_first() or safe_grab(response.meta, ["pc"])
